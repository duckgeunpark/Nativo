...
const TEMPLATES: Record<Language, Record<number, RoutineTask[]>> = {
  english: {
    1: [
...
/** 해당 언어/Phase 의 루틴 항목. (정의 없으면 Phase 1로 폴백) */
export function getRoutineTasks(language: Language, phase: number): RoutineTask[] {
  const byPhase = TEMPLATES[language];
  return byPhase[phase] ?? byPhase[1] ?? [];
}
