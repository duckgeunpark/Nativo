...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen">
      ...
      <nav
        aria-label="하단 탭"
        className="fixed inset-x-0 bottom-0 z-40 grid h-[calc(3.25rem+env(safe-area-inset-bottom))] grid-cols-6 border-t bg-card pb-[env(safe-area-inset-bottom)] lg:hidden"
      >
        {NAV.map((item) => {
          const active = isActive(pathname, item.match);
          const Icon = item.icon;
